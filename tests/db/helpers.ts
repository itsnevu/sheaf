...
/** Test fixtures against the SQLite test database. Each call creates an isolated desk (organisation). */

let counter = 0;

export async function makeOrg(overrides: Partial<{ requireFourEyes: boolean; maxRetries: number; treasuryAddress: string | null }> = {}) {
  const n = ++counter;
  const org = await db.organization.create({ data: { name: `Desk ${n}`, slug: `desk-${n}-${Date.now().toString(36)}`, requireFourEyes: overrides.requireFourEyes ?? true, maxRetries: overrides.maxRetries ?? 3, treasuryAddress: overrides.treasuryAddress ?? "0x00000000000000000000000000000000000000a1" } });
  ...
  // `finance` is the desk operator (role value FINANCE_ADMIN, label "Desk operator").
...
/** Leg CSV in the desk format: label,address,asset,amount,not_before,memo (asset USDG). */
export function csvOf(rows: Array<{ name: string; address: string; amount: string; reference?: string; notBefore?: string }>) {
  return "label,address,asset,amount,not_before,memo\n" + rows.map((r) => `${r.name},${r.address},USDG,${r.amount},${r.notBefore ?? ""},${r.reference ?? ""}`).join("\n") + "\n";
}
