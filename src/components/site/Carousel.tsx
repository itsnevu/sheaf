...
/** Horizontal, scroll-snapped strip with the reference's white chamfered arrow buttons. */
export default function Carousel({ children, label, className = "" }: { children: ReactNode; label: string; className?: string }) {
  ...
  return (
    <div className={`relative ${className}`}>
      <div ref={ref} className="n-carousel" role="region" aria-label={label} tabIndex={0}>
        {children}
      </div>
      <div className="pointer-events-none absolute inset-y-0 left-0 right-0 flex items-center justify-between px-2 md:-mx-8">
        <button type="button" className="n-arrow n-clip pointer-events-auto" onClick={() => go(-1)} disabled={!canPrev} aria-label="Previous">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" aria-hidden="true">
            <path d="M15 5l-7 7 7 7" />
          </svg>
        </button>
        <button type="button" className="n-arrow n-clip pointer-events-auto" onClick={() => go(1)} disabled={!canNext} aria-label="Next">
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" aria-hidden="true">
            <path d="M9 5l7 7-7 7" />
          </svg>
        </button>
