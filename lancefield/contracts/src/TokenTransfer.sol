...
/// @title TokenTransfer
/// @notice Outbound and inbound ERC-20 transfer helper, plus native ETH send, that turns any failure
///         into one explicit error.
/// @dev Robinhood Stock Tokens run compliance checks on both sender and receiver and revert when
///      either is not verified. A plain `transfer` would bubble the token's own error (or, for a
///      non-standard token, a low-level revert with no data), which is hard to read from a wallet
///      or an indexer. This library wraps the call so every failed payout surfaces as
///      `TransferRejected(token, to, amount, reason)` with the token's revert data attached.
///      Because the revert unwinds the whole transaction, no contract state is changed and the
///      funds stay where they were (still claimable or refundable by the usual paths).
library TokenTransfer {
    ...
    /// @notice A native ETH send was refused by the receiver. `reason` is the receiver's raw revert data.
    error NativeTransferRejected(address to, uint256 amount, bytes reason);
    ...
    /// @notice Send `amount` of native ETH to `to` with all remaining gas, reverting with
    ///         `NativeTransferRejected` on failure.
    function sendNative(address to, uint256 amount) internal {
        (bool ok, bytes memory ret) = to.call{value: amount}("");
        if (!ok) revert NativeTransferRejected(to, amount, ret);
    }
